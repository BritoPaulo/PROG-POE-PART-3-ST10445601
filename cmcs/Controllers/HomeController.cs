...
namespace cmcs.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult AdminDashboard()
        {
            ...
            // Add ViewBag values for statistics and debug informat
        ...
        public async Task<IActionResult> SubmitClaim(Claim claim, List<IFormFile> supportingDocs)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    claim.WorkflowStatus = "Submitted";
        ...
        public async Task<IActionResult> SendForReview(int id)
        {
            ...
            if (claim != null)
            {
                ...
                claim.WorkflowStatus = "Under Review";
                await _context.SaveChangesAsync();
                TempData["SuccessMessage"] = $"Claim #{id} sent for review.";
            }
            return RedirectToAction("AdminDashboard");
        }

        [HttpPost]
        public async Task<IActionResult> ApproveClaim(int id, string approvalNotes = "")
        {
            var claim = await _context.Claims.FindAsync(id);
            if (claim != null)
            {
                claim.Status = "Approved";
                claim.WorkflowStatus = "Approved";
                claim.ApprovalNotes = approvalNotes;
                claim.ReviewedBy = "Administrator";
                claim.ReviewedDate = DateTime.Now;
                await _context.SaveChangesAsync();

                // Update approved count cookie
                var approvedCount = int.Parse(Request.Cookies["ApprovedCount"] ?? "0") + 1;
                Response.Cookies.Append("ApprovedCount", approvedCount.ToString(), new CookieOptions
                {
        ...
        public async Task<IActionResult> RejectClaim(int id)
        {
            ...
            if (claim != null)
            {
                ...
                // Update rejected count cookie
                var rejectedCount = int.Parse(Request.Cookies["RejectedCount"] ?? "0") + 1;
                Response.Cookies.Append("RejectedCount", rejectedCount.ToString(), new CookieOptions
                {
                    Expires = DateTime.Now.AddDays(7),
                    HttpOnly = true,
                    Secure = true,
        ...
        public IActionResult TrackClaim(int id)
        {
            ...
            {
                statusHistory.Add($"{claim.ReviewedDate:yyyy-MM-dd HH:mm:ss} - Claim has been approved by {claim.ReviewedBy}.");
            }
            else if (claim.Status == "Rejected")
            {
                statusHistory.Add($"{claim.ReviewedDate:yyyy-MM-dd HH:mm:ss} - Claim has been rejected by {claim.ReviewedBy}.");
            }
        ...
        public IActionResult ReviewClaim(int id)
        {
            ...
            var validationErrors = _validationService.ValidateClaim(claim);
